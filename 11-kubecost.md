...
```bash
aws eks update-kubeconfig --region ap-south-1 --name ekswithmoula
```
...
```bash
eksctl utils associate-iam-oidc-provider --cluster ekswithmoula --approve
```
...
```
 **Description:**  
- Creates an IAM service account (`ebs-csi-controller-sa`) in the `kube-system` namespace for the EKS cluster named `ekswithmoula`.
...
```bash
eksctl create addon --name aws-ebs-csi-driver --cluster ekswithmoula \
   --service-account-role-arn $SERVICE_ACCOUNT_ROLE_ARN --force
```
 **Description:**  
- Creates the `aws-ebs-csi-driver` add-on in the `ekswithmoula` cluster.
