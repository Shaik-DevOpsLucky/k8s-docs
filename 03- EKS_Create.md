...
```bash
eksctl create cluster --name=ekswithmoula \
  --version 1.31 \
  --region=ap-south-1 \
...
```bash
aws eks describe-cluster --name ekswithmoula --region ap-south-1 --query "cluster.identity.oidc.issuer" --output text
```
...
```bash
aws iam list-open-id-connect-providers | grep $(aws eks describe-cluster --name ekswithmoula --region ap-south-1 --query "cluster.identity.oidc.issuer" --output text | sed 's|https://||')
```
...
```bash
eksctl delete cluster --name=ekswithmoula
```
